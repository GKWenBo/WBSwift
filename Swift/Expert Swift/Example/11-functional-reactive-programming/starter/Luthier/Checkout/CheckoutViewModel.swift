...
class CheckoutViewModel: ObservableObject {
  // Input/Bindings
  @Published var selectedShippingOption: ShippingOption = ShippingOption(name: "", duration: "", price: 0)
  ...
  init(info: CheckoutInfo) {
    ...
    let currencyAndRate = currency
      .flatMap { currency -> AnyPublisher<(Currency, Decimal), Never> in
        guard currency != .usd else {
          return Just((currency, 1.0))
            .eraseToAnyPublisher()
        }
        
        return self.currencyService.getExchangeRate(for: currency)
          .map { (currency, $0) }
    ...
    currencyAndRate.map { currency, rate in
    ...
    $shippingPrices
      ...
      .map { pricedOptions, selectedOption, isLoading in
        guard selectedOption.price != 0 else {
          return "Free"
        }
    ...
    // $isUpdatingCurrency
    Publishers.Merge(currency.dropFirst().map { _ in true}, currencyAndRate.map { _ in false})
      .assign(to: &$isUpdatingCurrency)
    
    let orderResponse = shouldOrder
      .flatMap { [weak self] _ -> AnyPublisher<Void, Never> in
        self.map {
          $0.guitarService.order($0.guitar)
        } ?? Empty().eraseToAnyPublisher()
      }
      .share()
    
    // $isOrdering
    Publishers.Merge(shouldOrder.map { true },
