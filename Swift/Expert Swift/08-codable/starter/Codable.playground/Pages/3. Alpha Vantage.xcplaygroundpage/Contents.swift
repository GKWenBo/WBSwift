...
struct Stock: Decodable {
    ...
    init(from decoder: Decoder) throws {
        ...
        let timesDictionry = try container.decode([String: [String: String]].self, forKey: timesKey)
        let timeKeys = timesDictionry.keys.compactMap(AnyCodingKey.init(stringValue:))
        
        let timeContainer = try container.nestedContainer(keyedBy: AnyCodingKey.self, forKey: timesKey)
        
        // updates 
        self.updates = try timeKeys.reduce(into: [Update]()) { partialResult, currentKey in
