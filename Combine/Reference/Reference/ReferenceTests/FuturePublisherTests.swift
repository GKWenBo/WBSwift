...
class FuturePublisherTests: XCTestCase {
    ...
    func testFuturePublisher() {
        ...
        let  expectation = XCTestExpectation(description: "testFuturePublisher")
        
        /// create the future publisher
        ...
        let cancellabe = sut.sink { (error) in
            expectation.fulfill()
    ...
    func testFuturePublisherShowingFailure() {
        ...
        /// create the future publisher
        let sut = Future<Bool, Error> { promise in
            self.asyncAPICall(sabotage: true) { (grantedAccess, error) in
                if let error = error {
                    promise(.failure(error))
                } else {
                    promise(.success(grantedAccess))
        ...
        let cancellable = sut.sink { (err) in
            XCTAssertNotNil(err)
            expectation.fulfill()
        } receiveValue: { (value) in
            XCTFail()
