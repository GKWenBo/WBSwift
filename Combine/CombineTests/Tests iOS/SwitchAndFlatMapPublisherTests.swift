...
class SwitchAndFlatMapPublisherTests: XCTestCase {
    ...
    func testBasicFlatMap_String_NeverPublisher() {
        let simpleControllerPublisher = PassthroughSubject<String, Never>()
        
        let cancellable = simpleControllerPublisher
            .flatMap { (someValue) in
        ...
        simpleControllerPublisher.send(oneFish)
        simpleControllerPublisher.send(twoFish)
        simpleControllerPublisher.send(redFish)
        simpleControllerPublisher.send(blueFish)
