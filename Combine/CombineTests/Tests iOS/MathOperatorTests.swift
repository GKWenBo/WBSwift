...
class MathOperatorTests: XCTestCase {
    ...
    func testMax() {
        ...
        let cancellable = passSubj
            ...
            .sink { (completion) in
                ...
                latestReceivedResults = responseValue
            }
        
        passSubj.send(1)
        XCTAssertNil(latestReceivedResults)
        passSubj.send(2)
        XCTAssertNil(latestReceivedResults)
        passSubj.send(completion: .finished)
        XCTAssertEqual(latestReceivedResults, 2)
    ...
    func testMaxWithClosure() {
        ...
        let cancellable = passSubj
            .max {
                return $0.property1 < $1.property1
            }
            .sink { (completion) in
