...
//  Tests iOS
//
//  Created by wenbo on 2020/12/28.
//
...
class RetryPublisherTests: XCTestCase {
    ...
    func testCatchError(){
        ...
        let cancellable = passSubj
            ...
            .sink { (someString) in
                print(".sink() received data \(someString)")
                XCTAssertEqual(someString, "one")
            }
        
        passSubj.send(completion: .failure(TestFailureCondition.invalidServerResponse))
